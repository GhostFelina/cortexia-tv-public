...
cd /d "%~dp0"
echo.
echo   [1/8] Kaynak listeler indiriliyor...
...
call :free turkey tr
echo   [2/8] Aday listeler birlestiriliyor...
...
echo   [3/8] Tum yayinlar test ediliyor (20-40 dk)...
...
echo   [4/8] Katalog + altyazi taramasi...
node build-catalog.mjs
node probe-cc.mjs
echo   [5/8] Yayin akisi (EPG) indiriliyor...
node build-epg.mjs
echo   [6/8] Radyo istasyonlari indiriliyor...
node fetch-radio.mjs
echo   [7/8] Radyo yayinlari test ediliyor (10-20 dk)...
for %%c in (us gb es mx ar tr) do node test-radio.mjs %%c
node build-radio.mjs
echo   [8/8] Sertifika taramasi...
node probe-tls.mjs
echo.
