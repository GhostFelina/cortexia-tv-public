...
cd /d "%~dp0"
echo.
echo   [1/9] Kaynak listeler indiriliyor...
...
call :free turkey tr
echo   [2/9] Aday listeler birlestiriliyor...
...
echo   [3/9] Tum yayinlar test ediliyor (20-40 dk)...
...
echo   [4/9] Katalog + altyazi taramasi...
node build-catalog.mjs
node probe-cc.mjs
echo   [5/9] Yayin akisi (EPG) indiriliyor...
node build-epg.mjs
echo   [6/9] Radyo istasyonlari indiriliyor...
node fetch-radio.mjs
echo   [7/9] Radyo yayinlari test ediliyor (10-20 dk)...
for %%c in (us gb es mx ar tr) do node test-radio.mjs %%c
node build-radio.mjs
echo   [8/9] Sertifika taramasi...
node probe-tls.mjs
echo   [9/9] Web oynatilabilirlik taramasi...
node probe-web.mjs
echo.
